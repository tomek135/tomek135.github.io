...
export class UopSummaryComponent implements OnInit {
  ...
  static FIRST_TAX_RATE: number = 12;
  static SECOND_TAX_RATE: number = 32;
  static MAX_BASE_PENSION_PART : number = 177660;
  static TOTAL_EMPLOYEE_PART : number = UopSummaryComponent.DISABILITY_EMPLOYEE_PART + UopSummaryComponent.PENSION_EMPLOYEE_PART + UopSummaryComponent.SICKNESS_EMPLOYEE_PART;
  static TOTAL_EMPLOYER_PART : number = UopSummaryComponent.DISABILITY_EMPLOYER_PART + UopSummaryComponent.PENSION_EMPLOYER_PART ;
  ...
  healthyContributions = {
    ...
    employeePart: UopSummaryComponent.HEALTHY_EMPLOYEE_PART, 
    employeeValue: 0
  };
    ...
    this.contributions = [
      {
        ...
        employeePart: UopSummaryComponent.PENSION_EMPLOYEE_PART, 
        employeeValue: this.skEmerytalnaPracownik, 
        employerPart: UopSummaryComponent.PENSION_EMPLOYER_PART, 
        employerValue: this.skEmerytalnaPracodawca,
      },
      {
        contribution: 'Rentowe', 
        employeePart: UopSummaryComponent.DISABILITY_EMPLOYEE_PART, 
        employeeValue: this.skRentowaPracownik, 
        employerPart: UopSummaryComponent.DISABILITY_EMPLOYER_PART, 
        employerValue: this.skRentowaPracodawca
      },
      {
        contribution: 'Chorobowe', 
        employeePart: UopSummaryComponent.SICKNESS_EMPLOYEE_PART, 
  ...
  get totalEmployeePart(){
    return UopSummaryComponent.TOTAL_EMPLOYEE_PART;
  }
  
  get totalEmployerPart(){
    return Math.round((UopSummaryComponent.TOTAL_EMPLOYER_PART+this.basedParameters.accidentContributionPercent)*100)/100;
  }
  ...
  private calculateTaxContributions_2022(){
    ...
    this.taxContributions.tax = Math.round(this.taxContributions.baseOfTax*UopSummaryComponent.FIRST_TAX_RATE/100);
  ...
  getMonthlyPensionPart(month_number: number){
    if(this.basedParameters.grossIncome>0){
      if(month_number*this.basedParameters.grossIncome<UopSummaryComponent.MAX_BASE_PENSION_PART){
        return this.skEmerytalnaPracownik;
      }
      else{
        if(month_number*this.basedParameters.grossIncome-UopSummaryComponent.MAX_BASE_PENSION_PART<this.basedParameters.grossIncome){
          return Math.round(UopSummaryComponent.PENSION_EMPLOYEE_PART*(month_number*this.basedParameters.grossIncome-UopSummaryComponent.MAX_BASE_PENSION_PART))/100;
        }
        else{
          return 0;
        }
  ...
  getMonthlyDisabilityPart(month_number: number){
    if(this.basedParameters.grossIncome>0){
      if(month_number*this.basedParameters.grossIncome<UopSummaryComponent.MAX_BASE_PENSION_PART){
        ...
        if(month_number*this.basedParameters.grossIncome-UopSummaryComponent.MAX_BASE_PENSION_PART<this.basedParameters.grossIncome){
          return Math.round(UopSummaryComponent.DISABILITY_EMPLOYEE_PART*(month_number*this.basedParameters.grossIncome-UopSummaryComponent.MAX_BASE_PENSION_PART))/100;
        }
        else{
          return 0;
        }
